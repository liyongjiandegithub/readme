...
void update_config(char *filename, char *buff)
{
    ...
    cfgfd = open(filename,O_WRONLY|O_CREAT,0);
    printf("DBG:%s: %d\n %d\n \n%s\n",__FUNCTION__,cfgfd,strlen(buff),buff);

    write(cfgfd,buff,strlen(buff));
    close(cfgfd);
